...
void setup() {  
  ...
  shiftWrite1 (1, ledStateGreen);
  shiftWrite1 (2, ledStateGreen);
  }
 ...
 void loop() {  
  ...
  int readingWhite = digitalRead(buttonWhite);  
  ...
  shiftWrite(4, ledStateYellow);
  shiftWrite1(3, ledStateYellow);
  lastButtonStateYellow = readingYellow;

   //White button 
  if (readingWhite != lastButtonStateWhite) {
    lastDebounceTime = millis();
  }
  if ((millis() - lastDebounceTime) > debounceDelay) {
    if (readingWhite != buttonStateWhite) {
      buttonStateWhite = readingWhite;
      if (buttonStateWhite == HIGH) {
        ledStateWhite = !ledStateWhite;
        strobeLights_turnedOn=true;
      }
    }
  }
   if(ledStateWhite==HIGH){
    timedActionW.check();
  }else{
    shiftWrite1(4, ledStateWhite);
    shiftWrite1(5, ledStateWhite);
    shiftWrite1(6, ledStateWhite);
    strobeLights_turnedOn=false;
  }
  lastButtonStateWhite = readingWhite;
